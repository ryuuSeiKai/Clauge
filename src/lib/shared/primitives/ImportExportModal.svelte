<script lang="ts">
  ...
  // Import state
  let importFormat: 'auto' | 'Synape' | 'postman' | 'curl' = $state('auto');
  ...
  let exportFormat: 'Synape' | 'curl' = $state('Synape');
  ...
  function detectFormat(text: string): 'Synape' | 'postman' | 'curl' {
    ...
    try {
      ...
      if (obj.format && typeof obj.format === 'string' && obj.format.startsWith('Synape/')) {
        return 'Synape';
      }
  ...
  async function handleImport() {
    ...
    try {
      ...
      if (format === 'curl') {
        ...
        const result = await cmd.importSynape(content);
        showToast(result.message, 'success');
      }
  ...
  async function handleExport() {
    ...
    try {
      if (exportFormat === 'curl') {
        ...
        downloadFile(json, exportCollectionId === '__all__' ? 'Synape-export' : getCollectionName(exportCollectionId), 'json', 'application/json');
  ...
  {#if activeTab === 'import'}
    <div class="ie-section">
      ...
      <select class="ie-select" bind:value={importFormat}>
        ...
        <option value="Synape">Synape JSON</option>
